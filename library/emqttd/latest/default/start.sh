...
echo '['$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")']:emqttd start'
...
while [ x$(/opt/emqttd/bin/emqttd_ctl status |grep 'is running') != x ]   
do  
	sleep 10
	echo '['$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")']:emqttd running'
done

echo '['$(date -u +"%Y-%m-%dT%H:%M:%S.%3NZ")']:emqttd stop'
